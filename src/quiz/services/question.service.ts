...
export class QuestionService {
  ...
  async createQuestion(quizId: string, content: string, correctAnswer: string, difficulty: string): Promise<Question> {
    try {
      ...
      const index = quiz.questions.length + 1; // Set the index based on the number of existing questions
    ...
    try {
      for (const questionId of questions) {
        ...
        for (const student of students) {
          student.questionsSolved = student.questionsSolved.filter((qId) => qId.toString() !== questionId.toString());
          await student.save();
        }
